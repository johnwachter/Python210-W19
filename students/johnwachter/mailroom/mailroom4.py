...
D
git commit mailroom4.py -m "changing report function"




 2019-02-16, Created File
...
def createreport2():
    ...
    for donor in donor_db:
        avg = sum(donor_db[donor])/len(donor_db[donor])
        reportstring += "{:20} | ${:10,.2f}| {:>4} {:<4}| ${:<10,.2f} \n".format(donor, sum(donor_db[donor]), len(donor_db[donor]), "gifts", avg)
    return reportstring

def displayreport(report = createreport2()):
    print(report)
    print("\n")
...
def responsedict(choice):
    data = {'1': sendthankyou, '2': displayreport(), '3': createthankyouletters, '4': quitprogram}
